...
//		conv_from_unicode3は他の二つより短い変換結果を得られる場合があるが長いテキストでは変換時間が非常に長くなってしまう可能性あり
	...
	int		len1 = conv_from_unicode1(NULL, maxbufsize, sbuf, total_length, bCharSize);
	int		len2 = conv_from_unicode2(NULL, maxbufsize, sbuf, total_length, bCharSize);
//	int		len3 = conv_from_unicode3(NULL, maxbufsize, sbuf, total_length, bCharSize);

	int		len;

	if(len1 <= len2) {
		len = conv_from_unicode1(dbuf, maxbufsize, sbuf, total_length, bCharSize);
	} else {
		len = conv_from_unicode2(dbuf, maxbufsize, sbuf, total_length, bCharSize);
	}
...
int conv_from_unicode1(unsigned char *dbuf, const int maxbufsize, const WCHAR *sbuf, const int total_length, const BOOL bCharSize)
{
...
int conv_from_unicode2(unsigned char *dbuf, const int maxbufsize, const WCHAR *sbuf, const int total_length, const BOOL bCharSize)
{
...
int conv_from_unicode3(unsigned char *dbuf, const int maxbufsize, const WCHAR *sbuf, const int total_length, const BOOL bCharSize)
{
...
int jis3combconv(const int jiscode, WCHAR* dbuf)
{
//	JIS第三水準漢字非漢字でunicode合成文字に対応するものについての jiscode -> unicode変換
...
//	dbuf		変換先buf
	...
	static WCHAR jis3combtable[] = 
	{
		#include "jis3combiningtable.h"
	};

	const WCHAR	code = (WCHAR)jiscode;

	void	*result = bsearch(&code, jis3combtable, sizeof(jis3combtable) / sizeof(WCHAR) / 3, sizeof(WCHAR) * 3, compareforwchar);

	if(result == NULL) return 0;

	dbuf[0] = *((WCHAR*)result + 1);
	dbuf[1] = *((WCHAR*)result + 2);

	return 2;
}


int jis3combrevconv(int *code, const WCHAR* sbuf)
{
//	JIS第三水準漢字非漢字でunicode合成文字に対応するものについての unicode -> jiscode変換
//	対応する文字コードがなければ戻り値0を返す
//
//	sbuf		変換元buf
//	戻り値		unicode合成文字長(= 2, WCHAR単位)
//	code		変換先buf
	...
	if(!bTableInitialized) {
		...
		qsort(jis3combrevtable, sizeof(jis3combrevtable) / sizeof(WCHAR) / 3, sizeof(WCHAR) * 3, compareforwchar2);
		bTableInitialized = TRUE;
	}

	void	*result = bsearch(sbuf, jis3combrevtable, sizeof(jis3combrevtable) / sizeof(WCHAR) / 3, sizeof(WCHAR) * 3, compareforwchar2);

	if(result == NULL) return 0;

	*code = (int)(*((WCHAR*)result + 2));
...
int tuikakigou2conv(const int code, WCHAR *dbuf)
{
//	追加記号集合のうち、対応するunicode文字が存在する文字が存在せず
//	[...]あるいは[#xx#xx]で表される文字についての jiscode -> unicode変換
//	対応する文字コードがなければ0を返す
//
//	jiscode		変換元jiscode
//	dbuf		変換先buf
//	戻り値		unicode文字長(WCHAR単位)

	static int	kigou2code[] =
	{
	...
	int			len;
	const int	*codep = &code;

	void	*result = bsearch(&codep, kigou2table, sizeof(kigou2table) / sizeof(void*) / 2, sizeof(void*) * 2, compareforkigou2code);
...
int tuikakigou2revconv(int *code, const WCHAR *sbuf)
{
//	追加記号集合のうち、対応するunicode文字が存在する文字が存在せず
//	[...]あるいは[#xx#xx]で表される文字についての unicode -> jiscode変換
//	対応する文字コードがなければ0を返す
//
//	sbuf		変換元buf
//	戻り値		unicode文字長(WCHAR単位)
//	code		変換されたjiscode
	...
	if(!bTableInitialized) {
		...
		qsort(kigou2revtable, sizeof(kigou2revtable) / sizeof(void*) / 2, sizeof(void*) * 2, compareforkigou2str);
		bTableInitialized = TRUE;
	}

	int		len;

	void	*result = bsearch(&sbuf, kigou2revtable, sizeof(kigou2revtable) / sizeof(void*) / 2, sizeof(void*) * 2, compareforkigou2str);

	if(result != NULL) {
		len = (int)wcslen(*(WCHAR**)result);
		*code = **((int**)result + 1);
	} else {
		if( (sbuf[0]==L'[')&&(sbuf[1]==L'#')&&(isdigit(sbuf[2])!=0)&&(isdigit(sbuf[3])!=0)&&(sbuf[4]==L'#')&&(isdigit(sbuf[5])!=0)&&(isdigit(sbuf[6])!=0)&&(sbuf[7]==L']') ) {
			*code = ((sbuf[2] - L'0') * 10 + (sbuf[3] - L'0')) * 256 + (sbuf[5] - L'0') * 10 + (sbuf[6] - L'0') + 0x2020;
			len = 8;
		} else {
			*code = 0;
			len = 0;
...
int compareforkigou2code(const void *item1, const void *item2)				// 変換テーブルソート，検索用関数
{
	return **(int**)item1 - **(int**)item2;
}


int compareforkigou2str(const void *item1, const void *item2)				// 変換テーブルソート，検索用関数
{
	WCHAR *str1 = *(WCHAR**)item1;
	WCHAR *str2 = *(WCHAR**)item2;

	int		i = 0;

	while( (str1[i] != 0x0000) && (str2[i] != 0x0000) ) {
...
int compareforwchar(const void *item1, const void *item2)					// 変換テーブルソート，検索用関数
{
	return (int)(*(WCHAR*)item1) - (int)(*(WCHAR*)item2);
}


int compareforwchar2(const void *item1, const void *item2)					// 変換テーブルソート，検索用関数
{
...
//		slen		結果のunicode文字長(WCHAR単位)
	...
	if(uc == L'[') {
		templen = tuikakigou2revconv(&code, sbuf);
		if(templen != 0) {
			*jiscode = code;
			*slen = templen;
			return F_TUIKAKIGOU;
	...
	templen = jis3combrevconv(&code, sbuf);
	if(templen != 0) {
		*jiscode = code;
		*slen = templen;
		return F_JIS1KANJI;
